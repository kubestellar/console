 ...
 */
export function extractSliceBody(goSource) {
 ...
 */
export function extractScoringConstants(goSource) {
  const constants = []
 ...
 */
export function splitStructLiterals(sliceBody) {
  const literals = []
  let depth = 0
  let current = ''
 ...
 */
export function parseStructLiteral(body) {
  const out = {}
...
export function renderFile(tiers, constants) {
  const header = [
...
// Only run main() when this file is invoked directly as a script.
// Under `import` (unit tests), skip execution so the pure helpers above
// can be tested without triggering the filesystem read of pkg/rewards/tiers.go.
