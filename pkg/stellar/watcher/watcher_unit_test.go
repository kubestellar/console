...
func (m *mockNotificationStore) CreateStellarNotification(ctx context.Context, n *store.StellarNotification) error {
m.mu.Lock()
defer m.mu.Unlock()
...
st := &mockNotificationStore{userIDs: []string{"user-1"}}
...
st := &mockNotificationStore{userIDs: []string{}} // no users
client := &mockK8sClient{
getEvents: func(ctx context.Context, cluster, namespace string, limit int) ([]k8s.Event, error) {
...
w := New(st, &mockK8sClient{}, 10*time.Second)
n := w.pollCluster(context.Background(), "test-cluster")
if n != 0 {
