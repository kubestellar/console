...
import (
	...
	handlersmcp "github.com/kubestellar/console/pkg/api/handlers/mcp"
...
func (s *Server) reloadOAuth(clientID, clientSecret string) {
	...
	s.auth.handler.SetSSECanceller(handlersmcp.CancelUserSSEStreams)
...
func (s *Server) setupAuthRoutes(app *fiber.App) *routeSetupContext {
	...
	auth.SetHub(s.hub)
	auth.SetSSECanceller(handlersmcp.CancelUserSSEStreams)
