...
func (w *GPUUtilizationWorker) scrapeDCGMPerCluster(
...
) map[string]map[string]*gpu.DCGMNamespaceMetrics {
	if !w.dcgmEnabled {
		return nil
	}
	...
	out := make(map[string]map[string]*gpu.DCGMNamespaceMetrics, len(clusters))
	scrapeConfig := gpu.DCGMScrapeConfig{
	...
	for cluster := range clusters {
		...
		metrics, err := gpu.ScrapeDCGMByNamespace(ctx, restConfig, scrapeConfig)
		cancel()
		if err != nil {
...
func (w *GPUUtilizationWorker) collectForReservation(
	...
	dcgmClusterMetrics map[string]*gpu.DCGMNamespaceMetrics,
) {
