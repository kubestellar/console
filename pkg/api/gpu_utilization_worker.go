...
func (w *GPUUtilizationWorker) scrapeDCGMPerCluster(
	...
	out := make(map[string]map[string]*gpu.NamespaceMetrics, len(clusters))
	scrapeConfig := gpu.ScrapeConfig{
	...
	for cluster := range clusters {
		...
		metrics, err := gpu.ScrapeByNamespace(ctx, restConfig, scrapeConfig)
		cancel()
		if err != nil {
...
func (w *GPUUtilizationWorker) collectForReservation(
	...
	if err != nil {
		...
		consolemetrics.RecordGPUUtilReservationCollect(consolemetrics.GPUUtilOutcomePodsError)
		return
	}
	...
	if err != nil {
		...
		consolemetrics.RecordGPUUtilReservationCollect(consolemetrics.GPUUtilOutcomeNodesError)
		return
	}
	...
	if err := w.store.InsertUtilizationSnapshot(ctx, snapshot); err != nil {
		...
		consolemetrics.RecordGPUUtilReservationCollect(consolemetrics.GPUUtilOutcomeSnapshotError)
		return
	}

	consolemetrics.RecordGPUUtilReservationCollect(consolemetrics.GPUUtilOutcomeSuccess)
}
