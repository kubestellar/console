...
// SSEBroadcaster is implemented by types that can broadcast an SSEEvent to
// all connected clients (used to decouple the Handler from the concrete
// broadcaster implementation).
type SSEBroadcaster interface {
	Broadcast(event SSEEvent)
}

// SSEEvent is a single server-sent event delivered to Stellar SSE clients.
