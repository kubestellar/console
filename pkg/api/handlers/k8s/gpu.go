...
type gpuProvisioningClient interface {
	...
	CreateOrUpdateResourceQuota(ctx context.Context, contextName string, spec k8sclient.ResourceQuotaSpec) (*k8sclient.ResourceQuota, error)
...
func (h *GPUHandler) CreateReservation(c *fiber.Ctx) error {
	...
	if err := httputil.ValidateClusterAndNamespace(input.Cluster, input.Namespace); err != nil {
		return err
	}
	if input.GPUCount < 1 {
...
func (h *GPUHandler) provisionOnCluster(ctx context.Context, r *models.GPUReservation) error {
	...
	quotaSpec := k8sclient.ResourceQuotaSpec{
		Name:      quotaName,
