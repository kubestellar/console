...
// namespaceClient is the narrow interface NamespaceHandler requires from the
// cluster layer. *k8sclient.MultiClusterClient satisfies it implicitly.
...
type NamespaceHandler struct {
	store     store.Store
	k8sClient namespaceClient
}

// NewNamespaceHandler creates a new namespace handler.
// Accepts the full *k8sclient.MultiClusterClient at the call-site but stores it as
// the narrow namespaceClient interface so tests can substitute a simple mock.
func NewNamespaceHandler(s store.Store, k8sClient *k8sclient.MultiClusterClient) *NamespaceHandler {
...
func (h *NamespaceHandler) ListNamespaces(c *fiber.Ctx) error {
	...
	if err := auth.RequireViewerOrAbove(c, h.store); err != nil {
		return err
	}
...
func (h *NamespaceHandler) GetNamespaceAccess(c *fiber.Ctx) error {
	...
	if err := httputil.ValidateK8sName("namespace", name); err != nil {
		return err
	}
	...
	accessList := make([]models.NamespaceAccessEntry, 0, len(bindings)*2)
