...
// serviceExportClient defines the narrow subset of k8sclient.MultiClusterClient used by ServiceExportHandlers.
type serviceExportClient interface {
	DeduplicatedClusters(ctx context.Context) ([]k8sclient.ClusterInfo, error)
	ListClusters(ctx context.Context) ([]k8sclient.ClusterInfo, error)
...
type ServiceExportHandlers struct {
	k8sClient serviceExportClient
}

// NewServiceExportHandlers creates a new ServiceExport handlers instance.
// Accepts *k8sclient.MultiClusterClient (or any serviceExportClient implementation).
func NewServiceExportHandlers(k8sClient *k8sclient.MultiClusterClient) *ServiceExportHandlers {
...
// "cluster failed" and display an appropriate degraded-state indicator. Defined
// here (rather than the root shared_types.go) since ServiceExportHandlers is
// currently its only user; the root handlers package keeps a type alias for
// backward compatibility (epic #23685).
