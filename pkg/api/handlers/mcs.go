...
// mcsK8sClient defines the narrow interface for MCS API operations.
// *k8s.MultiClusterClient satisfies this implicitly (Go structural typing).
type mcsK8sClient interface {
...
type MCSHandlers struct {
	k8sClient mcsK8sClient
	hub       *Hub
}

// NewMCSHandlers creates a new MCS handlers instance.
// Accepts *k8s.MultiClusterClient or any implementation of mcsK8sClient.
func NewMCSHandlers(k8sClient mcsK8sClient, hub *Hub) *MCSHandlers {
	return &MCSHandlers{
		k8sClient: k8sClient,
		hub:       hub,
	}
