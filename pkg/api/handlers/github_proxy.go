...
const (
	...
	// githubProxyAllowedReposEnv is the env var controlling which repositories
	// the shared GitHub proxy may access with the server-side token.
	githubProxyAllowedReposEnv = "GITHUB_PROXY_REPOS"
	// githubProxyAllowedReposDefault limits the proxy to KubeStellar repos unless
	// an operator explicitly expands the allowlist.
	githubProxyAllowedReposDefault = "kubestellar/kubestellar,kubestellar/console,kubestellar/docs,kubestellar/ocm-transport-plugin,kubestellar/galaxy,kubestellar/ui"
...
func getGitHubProxyAllowedRepos() map[string]struct{} {
	...
	if env := os.Getenv(githubProxyAllowedReposEnv); env != "" {
		envRepos := make([]string, 0)
		for _, repo := range strings.Split(env, ",") {
			normalized, ok := normalizeGitHubProxyRepo(repo)
			if !ok {
				slog.Warn("[GitHubProxy] invalid repo slug in allowlist env, skipping", "env", githubProxyAllowedReposEnv, "repo", strings.TrimSpace(repo))
				continue
			}
...
func (h *GitHubProxyHandler) Proxy(c *fiber.Ctx) error {
	...
	if !h.isAllowedGitHubPath(apiPath) {
		...
		audit.Log(c, audit.ActionUnauthorizedAttempt, "github_proxy_path", apiPath, "path not allowed", apiPath)
