...
func (h *FeedbackHandler) ListAllFeatureRequests(c *fiber.Ctx) error {
	...
	var (
		consoleIssues []GitHubIssue
		docsIssues    []GitHubIssue
		docsErr       error
	)

	g, ctx := errgroup.WithContext(c.UserContext())
	g.Go(func() error {
		issues, err := h.fetchGitHubIssuesFromRepo(ctx, currentGitHubLogin, h.repoName)
		if err != nil {
			return err
		}
	...
	g.Go(func() error {
		issues, err := h.fetchGitHubIssuesFromRepo(ctx, currentGitHubLogin, docsRepoName)
		if err != nil {
			docsErr = err
			return nil
		}
