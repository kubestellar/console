...
func TestHandleAIProcessingComplete_UpdatesStatusAndNotifies(t *testing.T) {
	...
	handler := &FeedbackHandler{
		...
		repoOwner: "kubestellar",
		repoName:  "console",
		// Provide a no-op HTTP client so getLatestBotComment doesn't panic on nil.
		httpClient: &http.Client{
			Transport: RoundTripFunc(func(req *http.Request) *http.Response {
				return &http.Response{
					StatusCode: http.StatusOK,
	...
	mockStore.AssertCalled(t, "UpdateFeatureRequestLatestComment", requestID, mock.AnythingOfType("string"))
