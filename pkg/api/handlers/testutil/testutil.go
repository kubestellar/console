...
// handlers/mcp, handlers/github, and handlers/gitops can import it directly.
// It intentionally exports only testing-support types and helpers.
package testutil
...
// SetupTestEnv creates a new test environment with a fresh Fiber app and
// an initialized SettingsManager pointing to a temporary directory.
...
// Used by tests that need to inject fake dynamic clients with typed objects.
func NewK8sScheme() *runtime.Scheme {
	scheme := runtime.NewScheme()
	// Register core/apps/batch types — same set as the workloads/mcp handlers expect.
	_ = k8sscheme.AddToScheme(scheme)
	return scheme
}

// InjectDynamicClusterWithObjects creates a fake dynamic client seeded with typed K8s
// objects and injects both dynamic and typed clients into the test environment.
