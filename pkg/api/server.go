...
func (s *Server) setupRoutes() {
	...
	// Shared handlers (missions, namespaces, complianceFrameworks) created once
	// in setupRoutes() and passed to helpers as needed across domain boundaries.
	missions := handlers.NewMissionsHandler()
	complianceFrameworks := handlers.NewComplianceFrameworksHandler(nil)
	dataResidency := handlers.NewDataResidencyHandler(residency.NewEngine())

	// #8862 rewarded contributions org filter configuration.
	rewardsHandler := handlers.NewRewardsHandler(handlers.RewardsConfig{
		GitHubToken: s.config.GitHubToken,
		Orgs:        s.config.RewardsGitHubOrgs,
	})

	// Public routes initialization helper.
	s.setupPublicRoutes(publicLimiter, publicAPI, missions, complianceFrameworks, dataResidency, rewardsHandler)
	...
	s.setupGitOpsRoutes(api, gitopsHandlers, selfUpgradeHandler)

	// K8s Resources helper routes (workloads, MCS, Gateway, CRDs, etc.).
	...
	s.setupK8sResourceRoutes(api, workloadHandlers)
