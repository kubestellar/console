...
func (s *Server) setupAPICoreRoutes(routes *routeSetupContext) {
	newAPICoreRouteGroup(s.app, s.store, s.config, s.hub, s.notificationService, s.persistenceStore, s.k8sClient, s.auth.failureTracker, s.lifecycle.done).Register(routes)
}
