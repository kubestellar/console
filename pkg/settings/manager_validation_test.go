...
func TestValidation_SaveAll_EmptySettings(t *testing.T) {
	...
	if loaded.AIMode != DefaultAllSettings().AIMode {
		t.Errorf("aiMode = %q, want default %q", loaded.AIMode, DefaultAllSettings().AIMode)
	}
	if loaded.Theme != DefaultAllSettings().Theme {
		t.Errorf("theme = %q, want default %q", loaded.Theme, DefaultAllSettings().Theme)
	}
}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !tc.wantErr {
				...
				if loaded.Predictions.Thresholds != tc.want {
					t.Errorf("thresholds = %+v, want %+v", loaded.Predictions.Thresholds, tc.want)
				}
			}
		})
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !tc.wantErr {
				...
				if loaded.TokenUsage != tc.want {
					t.Errorf("tokenUsage = %+v, want %+v", loaded.TokenUsage, tc.want)
				}
			}
		})
	}
