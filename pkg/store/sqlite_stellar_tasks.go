...
func (s *SQLiteStore) MarkObservationShown(ctx context.Context, id string, userID string) error {
	_, err := s.db.ExecContext(ctx, `UPDATE stellar_observations SET shown_to_user = 1 WHERE id = ? AND (user_id = ? OR user_id = '')`, id, userID)
	return err
}
