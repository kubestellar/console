...
// maxExtractFileSize caps the size of any single file during tar extraction
// to prevent decompression bombs.
const maxExtractFileSize = 256 * 1024 * 1024 // 256 MB

// extractTarGzSafe extracts a .tar.gz archive to destDir using Go's archive/tar,
// with explicit path-traversal validation on every entry (CWE-22, #16500).
func extractTarGzSafe(ctx context.Context, archivePath, destDir string) error {
	...
	for {
		...
		case tar.TypeSymlink, tar.TypeLink:
			...
			return fmt.Errorf("symlinks not allowed in update archive: %s", header.Name)
		default:
...
func safeTarExtract(ctx context.Context, archivePath, destDir string) error {
	...
	for {
		...
		target := filepath.Clean(filepath.Join(cleanDestDir, cleanName))
