...
// ErrorSanitizerFunc is a function that sanitizes error messages.
// This avoids importing the agent package (which would create a circular dependency).
type ErrorSanitizerFunc func(operation string, err error) string

// defaultSanitizer provides a basic error sanitizer when none is configured.
func defaultSanitizer(_ string, err error) string {
	return err.Error()
}

// SanitizeError is the error sanitizer used by cluster operations.
// Set this to the agent package's sanitizeAgentError at init time.
var SanitizeError ErrorSanitizerFunc = defaultSanitizer
...
func (k *Proxy) PreviewKubeconfig(yamlContent string) ([]KubeconfigPreviewEntry, error) {
	k.mu.RLock()
	defer k.mu.RUnlock()
...
func (k *Proxy) ImportKubeconfig(yamlContent string) (added []string, skipped []string, err error) {
	incoming, err := clientcmd.Load([]byte(yamlContent))
	if err != nil {
	...
	for name, ctx := range incoming.Contexts {
		...
		// Resolve cluster name collisions
		clusterName := ctx.Cluster
		...
		// Resolve user/auth-info name collisions
		userName := ctx.AuthInfo
...
func (k *Proxy) AddCluster(req AddClusterRequest) error {
	k.mu.Lock()
	defer k.mu.Unlock()
...
func (k *Proxy) TestClusterConnection(req TestConnectionRequest) (*TestConnectionResult, error) {
	if req.ServerURL == "" {
	...
	if err != nil {
		return &TestConnectionResult{Reachable: false, Error: SanitizeError("create client", err)}, nil
	}

	version, err := client.Discovery().ServerVersion()
	if err != nil {
		return &TestConnectionResult{Reachable: false, Error: SanitizeError("test connection", err)}, nil
	}
