...
func (s *Server) handleChatMessage(msg protocol.Message, forceAgent string, parentCtx ...context.Context) protocol.Message {
	...
	// SECURITY: Enforce prompt size limit to prevent resource exhaustion (CWE-770, #16759).
	// The streaming path already enforces this; the non-streaming path must match.
