...
// LocalOpenAICompatProvider implements ai.Provider for a family of local LLM
...
//
// These providers expose ai.CapabilityChat only — they cannot shell out to
...
func (p *LocalOpenAICompatProvider) localOpenAICompatBaseURL() string {
	...
	if v := strings.TrimRight(config.GetConfigManager().GetBaseURL(p.providerKey), "/"); v != "" {
		return v
	}
...
func (p *LocalOpenAICompatProvider) Capabilities() ai.ProviderCapability {
...
func (p *LocalOpenAICompatProvider) Chat(ctx context.Context, req *ai.ChatRequest) (*ai.ChatResponse, error) {
	endpoint := p.endpoint()
	if endpoint == "" {
...
// apply as Chat above.
func (p *LocalOpenAICompatProvider) StreamChat(ctx context.Context, req *ai.ChatRequest, onChunk func(chunk string)) (*ai.ChatResponse, error) {
	endpoint := p.endpoint()
	if endpoint == "" {
		return nil, fmt.Errorf("%s URL not configured (set %s)", p.displayName, p.urlEnvVar)
	}
