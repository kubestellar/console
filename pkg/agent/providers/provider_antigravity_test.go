...
func TestAntigravityProvider_HandshakeInterface(t *testing.T) {
	// Verify AntigravityProvider implements ai.HandshakeProvider
	var _ ai.HandshakeProvider = &AntigravityProvider{}
}

func TestAntigravityProvider_HandshakeResult_Fields(t *testing.T) {
	// Test that ai.HandshakeResult has the expected fields
	r := &ai.HandshakeResult{
		Ready:         true,
		State:         "connected",
...
func TestAntigravityProvider_StreamChat_RejectsDesktopForbiddenRequests(t *testing.T) {
	...
	req := &ai.ChatRequest{Prompt: "Do not open the desktop app. Stay in the terminal and run kind create cluster --name demo."}
