...
// VSCodeProvider implements the ai.Provider interface for VS Code
...
func (v *VSCodeProvider) IsAvailable() bool {
	return v.appDetected || config.GetConfigManager().IsKeyAvailable("vscode")
}
func (v *VSCodeProvider) Capabilities() ai.ProviderCapability { return ai.CapabilityChat }

func (v *VSCodeProvider) Chat(ctx context.Context, req *ai.ChatRequest) (*ai.ChatResponse, error) {
	if !config.GetConfigManager().HasAPIKey("vscode") {
		return &ai.ChatResponse{Content: "VS Code is detected but no API key is configured. Set VSCODE_API_KEY to enable chat.", Agent: v.Name(), Done: true}, nil
	}
	return chatViaOpenAICompatible(ctx, req, "vscode", "https://api.github.com/copilot/chat/completions", v.Name())
}

func (v *VSCodeProvider) StreamChat(ctx context.Context, req *ai.ChatRequest, onChunk func(chunk string)) (*ai.ChatResponse, error) {
	if !config.GetConfigManager().HasAPIKey("vscode") {
		msg := "VS Code is detected but no API key is configured. Set VSCODE_API_KEY to enable chat."
		if onChunk != nil { onChunk(msg) }
		return &ai.ChatResponse{Content: msg, Agent: v.Name(), Done: true}, nil
	}
