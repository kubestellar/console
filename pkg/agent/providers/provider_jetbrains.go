...
// JetBrainsProvider implements the ai.Provider interface for JetBrains IDEs
...
func (j *JetBrainsProvider) IsAvailable() bool {
	return j.appDetected || config.GetConfigManager().IsKeyAvailable("jetbrains")
}
func (j *JetBrainsProvider) Capabilities() ai.ProviderCapability { return ai.CapabilityChat }

func (j *JetBrainsProvider) Chat(ctx context.Context, req *ai.ChatRequest) (*ai.ChatResponse, error) {
	if !config.GetConfigManager().HasAPIKey("jetbrains") {
		return &ai.ChatResponse{Content: "JetBrains IDE is detected but no API key is configured. Set JETBRAINS_API_KEY to enable chat.", Agent: j.Name(), Done: true}, nil
	}
	return chatViaOpenAICompatible(ctx, req, "jetbrains", "https://api.jetbrains.ai/v1/chat/completions", j.Name())
}

func (j *JetBrainsProvider) StreamChat(ctx context.Context, req *ai.ChatRequest, onChunk func(chunk string)) (*ai.ChatResponse, error) {
	if !config.GetConfigManager().HasAPIKey("jetbrains") {
		...
		return &ai.ChatResponse{Content: msg, Agent: j.Name(), Done: true}, nil
	}
