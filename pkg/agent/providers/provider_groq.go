...
func groqResolveBaseURL() string {
	...
	if v := config.GetConfigManager().GetBaseURL(groqProviderKey); v != "" {
		return v
	}
...
// GroqProvider implements ai.Provider for Groq (https://groq.com).
//
...
func (g *GroqProvider) IsAvailable() bool {
	...
	return config.GetConfigManager().IsKeyAvailable(groqProviderKey)
}

func (g *GroqProvider) Capabilities() ai.ProviderCapability {
...
func (g *GroqProvider) Chat(ctx context.Context, req *ai.ChatRequest) (*ai.ChatResponse, error) {
...
func (g *GroqProvider) StreamChat(ctx context.Context, req *ai.ChatRequest, onChunk func(chunk string)) (*ai.ChatResponse, error) {
