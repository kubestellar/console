...
func TestKagentiProvider_Interfaces(t *testing.T) {
	var _ ai.Provider = &KagentiProvider{}
	var _ ai.StreamingProvider = &KagentiProvider{}
	var _ ai.HandshakeProvider = &KagentiProvider{}
}
...
func TestKagentiProvider_IsAvailable_NoAgent(t *testing.T) {
	...
	if p.Capabilities() != ai.CapabilityChat|ai.CapabilityToolExec {
...
func TestKagentiProvider_DirectAgent_StreamChat(t *testing.T) {
	...
	res, err := p.Chat(ctx, &ai.ChatRequest{Prompt: "hi", SessionID: "s1"})
	if err != nil {
...
func TestKagentiProvider_BuildPromptIncludesK8sContext(t *testing.T) {
	p := &KagentiProvider{}
	prompt := p.buildPrompt(&ai.ChatRequest{
