...
// estimateChatTokenUsage builds a ai.ProviderTokenUsage from the request
...
func estimateChatTokenUsage(req *ai.ChatRequest, responseContent string) *ai.ProviderTokenUsage {
	if req == nil {
		// Without a request we can still attribute output tokens.
		out := estimateTokensFromText(responseContent)
		return &ai.ProviderTokenUsage{
			InputTokens:  0,
			OutputTokens: out,
			TotalTokens:  out,
		}
...
func collectExplicitNegativeConstraints(req *ai.ChatRequest) []string {
	if req == nil {
		return nil
	}
...
func buildExplicitNegativeConstraintBlock(req *ai.ChatRequest) string {
...
func requestForbidsDesktopCompanion(req *ai.ChatRequest) bool {
	return len(collectExplicitNegativeConstraints(req)) > 0
}

// buildPromptWithHistoryGeneric creates a prompt string from a ai.ChatRequest
// including system prompt and conversation history.
// Used by CLI-based providers that take a single prompt string.
func buildPromptWithHistoryGeneric(req *ai.ChatRequest) string {
	var sb strings.Builder
