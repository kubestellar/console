...
// OpenWebUIProvider implements the ai.Provider interface for Open WebUI.
//
...
func openWebUIResolveBaseURL() string {
	...
	if v := config.GetConfigManager().GetBaseURL("open-webui"); v != "" {
		return v
	}
	return ""
}
...
func (o *OpenWebUIProvider) IsAvailable() bool {
	return o.getEndpoint() != "" && config.GetConfigManager().IsKeyAvailable("open-webui")
}

func (o *OpenWebUIProvider) Capabilities() ai.ProviderCapability { return ai.CapabilityChat }
...
func (o *OpenWebUIProvider) Chat(ctx context.Context, req *ai.ChatRequest) (*ai.ChatResponse, error) {
...
func (o *OpenWebUIProvider) StreamChat(ctx context.Context, req *ai.ChatRequest, onChunk func(chunk string)) (*ai.ChatResponse, error) {
	endpoint := o.getEndpoint()
	if endpoint == "" {
		return nil, fmt.Errorf("Open WebUI URL not configured (set OPEN_WEBUI_URL)")
	}
