...
// KagentiProvider implements ai.Provider and ai.StreamingProvider for Kagenti agents REST API.
...
var _ ai.Provider = (*KagentiProvider)(nil)
var _ ai.StreamingProvider = (*KagentiProvider)(nil)
var _ ai.HandshakeProvider = (*KagentiProvider)(nil)
...
func (p *KagentiProvider) Capabilities() ai.ProviderCapability {
	return ai.CapabilityChat | ai.CapabilityToolExec
}

func (p *KagentiProvider) Handshake(ctx context.Context) *ai.HandshakeResult {
	if p.client == nil {
...
func (p *KagentiProvider) StreamChat(ctx context.Context, req *ai.ChatRequest, onChunk func(chunk string)) (*ai.ChatResponse, error) {
	return p.StreamChatWithProgress(ctx, req, onChunk, nil)
}

func (p *KagentiProvider) buildPrompt(req *ai.ChatRequest) string {
	var sb strings.Builder

	if req.SystemPrompt != "" {
		sb.WriteString(req.SystemPrompt)
	} else {
		sb.WriteString(prompts.DefaultSystemPrompt)
	}
...
func (p *KagentiProvider) StreamChatWithProgress(ctx context.Context, req *ai.ChatRequest, onChunk func(chunk string), onProgress func(event ai.StreamEvent)) (*ai.ChatResponse, error) {
	if p.client == nil {
			...
			if jsonErr := json.Unmarshal([]byte(data), &eventObj); jsonErr == nil {
				if t, ok := eventObj["type"].(string); ok && t != "" {
					if t == "text" || t == "message_delta" {
						...
						onProgress(ai.StreamEvent{Type: t})
					}
				} else {
...
func (p *KagentiProvider) Chat(ctx context.Context, req *ai.ChatRequest) (*ai.ChatResponse, error) {
