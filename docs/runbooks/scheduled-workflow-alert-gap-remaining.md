# Remaining Scheduled Workflows — No Failure Alert Runbook (RESOLVED)
...
**Fixed.** PR [#23525](https://github.com/kubestellar/console/pull/23525) (merged)
added all 12 `name:` values below to the `workflows:` catch-all in
`.github/workflows/workflow-failure-issue.yml`, alongside `"Stale Issues"`,
`"MTTR Badge"`, `"Nightly DAST Security Scan"`, and `"In-Place Upgrade Smoke"`.
A scheduled failure of any workflow in this list now opens/updates a tracked
issue automatically via that catch-all — no manual `gh run list` polling is
required to detect one. Tracking issue
[#23144](https://github.com/kubestellar/console/issues/23144) is closed.
The tables and detection commands below are kept for historical reference and
as a manual fallback if the catch-all itself is ever suspected of failing.
...
13 (`upgrade-smoke.yml`) already had its own dedicated runbook because of its severity
("gold-standard canary for the self-upgrade mechanism"). This runbook covered the
...
PR #23525 added all 12 `name:` values above (plus `"In-Place Upgrade Smoke"`,
`"Console Live Promote"`, and `"Stale Issues"`, which have their own dedicated
runbooks) to the `workflows:` list in
`.github/workflows/workflow-failure-issue.yml` in a single pass, reusing the
catch-all's existing dedup-by-title-and-label and comment-on-recurring-failure
logic. No change to any of the 12 workflow files themselves was needed.
...
environment. If the catch-all itself stops firing (e.g. a future edit
accidentally drops one of these `name:` entries), use the manual detection
commands above to confirm and re-add the missing entry.
