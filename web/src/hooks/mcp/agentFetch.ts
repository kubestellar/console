...
// Best-effort migration cleanup for legacy tokens left in localStorage/sessionStorage.
// The agent token is now memory-only to prevent XSS exfiltration (CWE-922, #16903).
...
export function getStoredAgentToken(): string {
  ...
  // One-time migration: remove tokens from any web storage (legacy).
