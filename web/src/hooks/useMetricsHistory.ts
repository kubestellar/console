...
export function useMetricsHistory() {
  ...
  // Stable refs for volatile data — prevents interval reset on every MCP poll
  const clustersRef = useRef(clusters)
  const podIssuesRef = useRef(podIssues)
  const gpuNodesRef = useRef(gpuNodes)

  // Keep refs in sync with latest render data (does NOT trigger re-renders)
  ...
  // Uses refs so the interval starts once on mount and never resets
  useEffect(() => {
    ...
    const captureSnapshot = () => {
      ...
      // Read current data from stable refs (not stale closure variables)
    ...
    // Set up interval
    const intervalId = setInterval(captureSnapshot, intervalMs)
  ...
  // Manually trigger a snapshot (reads from refs for consistency)
  const captureNow = () => {
    const currentClusters = clustersRef.current
    const currentPodIssues = podIssuesRef.current
    const currentGpuNodes = gpuNodesRef.current
