...
export interface MarketplaceItemActionProps {
  item: MarketplaceItem
  onInstall: (item: MarketplaceItem) => void | Promise<void>
  onRemove: (item: MarketplaceItem) => void | Promise<void>
  isInstalled: boolean
}
              ...
              <>
                ...
                {item.type === 'theme' && item.themeColors ? (
                  <div className="flex gap-0.5">
                    {(item.themeColors || []).slice(0, MAX_THEME_COLORS).map((color, index) => (
                      <div
                        key={index}
...
export function DifficultyBadge({
  ...
  difficulty: NonNullable<MarketplaceItem['difficulty']>
}) {
  ...
  return (
    <span className={`flex items-center gap-1 text-2xs font-medium px-1.5 py-0.5 rounded ${config.color}`}>
      {Array.from({ length: config.stars }).map((_, index) => (
        <Star key={index} className="w-2.5 h-2.5 fill-current" />
      ))}
      {config.label}
    </span>
  )
