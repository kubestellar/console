...
import { AuthorBadge, DifficultyBadge, type MarketplaceItemActionProps } from './MarketplaceCards'
import {
  ISSUES_URL,
  MATURITY_CONFIG,
  TYPE_LABELS,
...
export function MarketplaceRow({
  ...
  onRemove,
  isInstalled,
}: MarketplaceItemActionProps) {
  const [installing, setInstalling] = useState(false)
  const [removing, setRemoving] = useState(false)
  const isHelpWanted = item.status === 'help-wanted'
  const typeInfo = TYPE_LABELS[item.type]

  const handleInstall = async (marketplaceItem: MarketplaceItem) => {
    setInstalling(true)
    try {
      await onInstall(marketplaceItem)
    } finally {
      setInstalling(false)
    }
  }

  const handleRemove = async (marketplaceItem: MarketplaceItem) => {
    setRemoving(true)
    try {
      await onRemove(marketplaceItem)
    } finally {
      setRemoving(false)
    }
  }
