    ...
    return (
      <div className="flex gap-2">
        <input
          ...
          type="text"
          value={namespace}
          onChange={e => onNamespaceChange(e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, ''), true)}
  ...
  return (
    <>
      <select
        ...
        disabled={isEditing || !cluster || (namespacesLoading && clusterNamespaces.length === 0)}
      ...
      {cluster && namespacesLoading && (
        <div className="mt-2 flex items-center gap-2 text-xs text-muted-foreground">
          <Loader2 className="h-3 w-3 animate-spin" />
          <span>{t('common:status.loadingNamespaces')}</span>
        </div>
      )}
      {cluster && namespacesError && !namespacesLoading && (
    ...
    <>
      ...
      <div>
        ...
        <NamespaceField
          ...
          onCancelNew={onCancelNewNamespace}
          onRetryNamespaces={onRetryNamespaces}
        />
      </div>
    </>
  )
