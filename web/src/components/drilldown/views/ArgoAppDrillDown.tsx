...
5. Recommend best practices for this deployment`
  ...
  const TABS: { id: TabType; label: string; icon: typeof Info }[] = [
    ...
    { id: 'gitops', label: 'GitOps Restart', icon: RefreshCw },
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <h4 className="text-sm font-medium text-foreground flex items-center gap-2">
                ...
                Declarative GitOps Deployment Restart
              </h4>
              <a
            ...
            <div className="p-4 rounded-lg border border-border bg-card/50 space-y-3">
              <div className="flex items-center justify-between">
                <div>
                  <h5 className="text-sm font-medium text-foreground">Trigger Argo CD Sync</h5>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Reconcile the live cluster state with the desired state in Git.
                    {syncStatus === 'OutOfSync' && (
                      <span className="ml-1 text-yellow-400 font-medium">This application is currently out of sync.</span>
                    )}
                  </p>
                </div>
                <button
            ...
            <div className="p-4 rounded-lg border border-border bg-card/50 space-y-3">
              <h5 className="text-sm font-medium text-foreground flex items-center gap-2">
                ...
                Declarative Restart via Annotation
              </h5>
              <p className="text-xs text-muted-foreground">
                To trigger a rolling restart declaratively, update the{' '}
                <code className="px-1 py-0.5 rounded bg-secondary text-xs font-mono">
                  kubectl.kubernetes.io/restartedAt
                </code>{' '}
                annotation in your deployment manifest and commit to Git. Argo CD will detect the change and apply it to the cluster.
              </p>
              <div className="relative group">
                <pre className="p-3 rounded-lg bg-secondary/50 border border-border text-xs font-mono text-foreground overflow-x-auto">
{`# In your Git repository — update this annotation to trigger a restart
apiVersion: apps/v1
kind: Deployment
metadata:
  name: ${appName || '<deployment-name>'}
  namespace: ${namespace}
spec:
  template:
    metadata:
...
`}
              ...
              <p className="text-xs text-muted-foreground">
                After committing, either wait for Argo CD auto-sync or click{' '}
                <strong className="text-orange-400">Sync Now</strong> above to apply immediately.
              </p>
            </div>
            ...
            <div className="p-4 rounded-lg border border-orange-500/20 bg-orange-500/5 space-y-3">
              <h5 className="text-sm font-medium text-foreground flex items-center gap-2">
                ...
                Why Declarative Restart?
              </h5>
              <ul className="space-y-1.5 text-xs text-muted-foreground">
                <li className="flex items-start gap-2">
                  <CheckCircle className="w-3.5 h-3.5 text-green-400 flex-shrink-0 mt-0.5" />
                  <span>All changes are version-controlled and auditable via Git history.</span>
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle className="w-3.5 h-3.5 text-green-400 flex-shrink-0 mt-0.5" />
                  <span>Rollouts, restarts, and configuration changes are triggered by Git commits — not manual commands.</span>
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle className="w-3.5 h-3.5 text-green-400 flex-shrink-0 mt-0.5" />
                  <span>Works across multi-cluster workloads managed by KubeStellar and Argo CD.</span>
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle className="w-3.5 h-3.5 text-green-400 flex-shrink-0 mt-0.5" />
                  <span>Enterprise-grade compliance — no imperative commands scattered across clusters.</span>
                </li>
              </ul>
