      ...
      >
        {isEditing ? (
          ...
          <>
            ...
            {!isCollapsed && canDrag && (
              <span className="absolute right-5 top-1/2 -translate-y-1/2 z-10 flex items-center gap-1 rounded bg-background/80 px-1 opacity-0 pointer-events-none backdrop-blur-xs transition-opacity group-hover:opacity-100 group-hover:pointer-events-auto group-focus-within:opacity-100 group-focus-within:pointer-events-auto">
                {!PROTECTED_SIDEBAR_IDS.includes(item.id) && (
                  <button
                    type="button"
                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); onRemove(item.id) }}
                    className="min-h-11 min-w-11 p-1 rounded text-muted-foreground/50 transition-colors hover:bg-red-500/20 hover:text-red-400"
                ...
                <span
                  className="min-h-11 min-w-11 cursor-grab rounded p-1 text-muted-foreground/50 transition-colors hover:bg-secondary hover:text-muted-foreground active:cursor-grabbing"
                  aria-hidden="true"
