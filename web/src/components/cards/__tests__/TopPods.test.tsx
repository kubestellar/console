...
})\ndescribe('TopPods hook ordering', () => {\n  it('calls useMemo before early return (hook ordering fix)', () => {\n    // Arrange: showSkeleton triggers early return\n    mockUseCardLoadingState.mockReturnValue({ showSkeleton: true, showEmptyState: false, hasData: false, isRefreshing: false })\n    // Provide pods data to ensure useMemo is called\n    const podsData = [\n      { name: 'pod1', restarts: 2, cpuUsageMillis: 100, cpuRequestMillis: 200, memoryUsageBytes: 1000000, memoryRequestBytes: 2000000, gpuRequest: 1, metricsAvailable: true },\n      { name: 'pod2', restarts: 1, cpuUsageMillis: 150, cpuRequestMillis: 250, memoryUsageBytes: 1500000, memoryRequestBytes: 2500000, gpuRequest: 0, metricsAvailable: true }\n    ]\n    mockPods.mockReturnValue({ pods: podsData, isLoading: false, isRefreshing: false, isDemoFallback: false, isFailed: false, consecutiveFailures: 0, error: null, lastRefresh: Date.now() })\n    // Act: render should not throw\n    expect(() => render(<TopPods />)).not.toThrow()\n    // No assertion on useMemo directly, but test passes if no hook error is thrown\n  })\n})\n
