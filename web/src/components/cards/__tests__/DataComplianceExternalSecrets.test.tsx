import React from 'react'
import { describe, it, expect, vi, beforeEach } from 'vitest'
import { render, screen, act } from '@testing-library/react'
import { ExternalSecrets } from '../DataComplianceExternalSecrets'

// ── Mocks ─────────────────────────────────────────────────────────────────────

vi.mock('react-i18next', () => ({
  initReactI18next: { type: '3rdParty', init: () => {} },
  useTranslation: () => ({ t: (k: string) => k }),
}))

const mockIsDemoMode = vi.fn(() => false)
vi.mock('../../../hooks/useDemoMode', async (importOriginal) => ({
  ...(await importOriginal<typeof import('../../../hooks/useDemoMode')>()),
  useDemoMode: () => ({ isDemoMode: mockIsDemoMode(), toggleDemoMode: vi.fn(), setDemoMode: vi.fn() }),
}))

const mockClusters = vi.fn(() => [])
vi.mock('../../../hooks/useMCP', () => ({
  useClusters: () => {
    const clusters = mockClusters()
    return { clusters, deduplicatedClusters: clusters }
  },
}))

const mockKubectlExec = vi.fn()
vi.mock('../../../lib/kubectlProxy', () => ({
  kubectlProxy: { exec: (...args: unknown[]) => mockKubectlExec(...args) },
}))

const mockUseCardLoadingState = vi.fn()
vi.mock('../CardDataContext', () => ({
  useCardLoadingState: (opts: unknown) => mockUseCardLoadingState(opts),
  useCardDemoState: () => ({ shouldUseDemoData: mockIsDemoMode(), reason: null, showDemoBadge: false }),
}))

vi.mock('../../../lib/constants/network', () => ({
  KUBECTL_DEFAULT_TIMEOUT_MS: 5000,
  METRICS_SERVER_TIMEOUT_MS: 3000,
  DEFAULT_REFRESH_INTERVAL_MS: 30000,
}))

// ── Tests ─────────────────────────────────────────────────────────────────────

describe('DataComplianceExternalSecrets (ExternalSecrets)', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mockIsDemoMode.mockReturnValue(false)
    mockClusters.mockReturnValue([])
    mockUseCardLoadingState.mockReturnValue({})
    mockKubectlExec.mockResolvedValue({ exitCode: 1, output: '' })
  })

  it('renders without crashing', async () => {
    await act(async () => render(<ExternalSecrets />))
    expect(document.body).toBeTruthy()
  })

  it('calls useCardLoadingState during render', async () => {
    await act(async () => render(<ExternalSecrets />))
    expect(mockUseCardLoadingState).toHaveBeenCalled()
  })

  describe('demo / no clusters', () => {
    it('shows not-installed UI when in demo mode', async () => {
      mockIsDemoMode.mockReturnValue(true)
      await act(async () => render(<ExternalSecrets />))
      expect(screen.getByText('External Secrets Integration')).toBeInTheDocument()
    })

    it('shows no-clusters message when no clusters connected', async () => {
      mockClusters.mockReturnValue([])
      await act(async () => render(<ExternalSecrets />))
      expect(screen.getByText('No clusters connected')).toBeInTheDocument()
    })

    it('shows install guide link', async () => {
      mockClusters.mockReturnValue([])
      const { container } = render(<ExternalSecrets />)
      await act(async () => {})
      const link = container.querySelector('a[href*="external-secrets.io"]')
      expect(link).toBeTruthy()
    })
  })

  describe('reachable clusters — ESO not detected', () => {
    const cluster = { name: 'prod', reachable: true }

    beforeEach(() => {
      mockClusters.mockReturnValue([cluster])
      mockKubectlExec.mockResolvedValue({ exitCode: 1, output: '' })
    })

    it('shows not-installed message after scan', async () => {
      await act(async () => render(<ExternalSecrets />))
      expect(screen.getByText(/no ESO installation detected/i)).toBeInTheDocument()
    })
  })

  describe('fetch error', () => {
    it('shows error UI when kubectl throws', async () => {
      const cluster = { name: 'prod', reachable: true }
      mockClusters.mockReturnValue([cluster])
      mockKubectlExec.mockRejectedValue(new Error('network error'))
      await act(async () => render(<ExternalSecrets />))
      // After error: shows not-installed / no clusters text depending on state
      expect(document.body).toBeTruthy()
    })
  })
})
