...
function appStatusLabel(status: KubeVelaApplication['status']): string {
  const labels: Record<KubeVelaApplication['status'], string> = {
    running: 'Running',
    workflowSuspending: 'Suspended',
    ...
    <div className="h-full flex flex-col min-h-card content-loaded gap-4 overflow-hidden">
      ...
      <div className="flex gap-3">
        <MetricTile
          ...
          value={`${data.apps.running}/${data.apps.total}`}
          colorClass={
            data.apps.failed > 0
              ? 'text-yellow-400'
              : data.apps.running === data.apps.total && data.apps.total > 0
                ? 'text-green-400'
        ...
        <MetricTile
          ...
          value={data.totalComponents > 0 ? data.totalComponents.toString() : '—'}
          colorClass="text-blue-400"
          icon={<Cpu className="w-3 h-3" />}
        />
        <MetricTile
          label={t('kubevela.traits', 'Traits')}
          value={data.totalTraits > 0 ? data.totalTraits.toString() : '—'}
