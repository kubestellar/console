...
import type { DeployMission, DeployClusterStatus } from '../../hooks/useDeployMissions'
import type { DeployedDep } from '../../lib/cardEvents'
import type { StatusConfig, ClusterStatusConfig, OrbitStatus } from './useMissionsData'
  ...
  return (
    <div className={cn(
      ...
      >
        ...
        <StatusIcon className={cn('w-4 h-4 shrink-0', config.color, config.animateClass)} />
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium text-foreground truncate">{mission.workload}</span>
            {mission.groupName && (
              <span className="text-2xs px-1.5 py-0.5 rounded bg-muted text-muted-foreground">{mission.groupName}</span>
            )}
          </div>
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <span className={cn('text-2xs px-1.5 py-0.5 rounded font-medium', config.bg, config.color)}>
            {config.label}
          </span>
        </div>
      </div>
      ...
      {mission.status === 'orbit' && orbitStatus && (
        <div className="px-3 pb-2 flex items-center gap-1.5">
          ...
          <span className="text-2xs text-muted-foreground">{orbitStatus.cadence} maintenance</span>
          ...
          {orbitStatus.overdue && <span className="text-2xs font-medium text-amber-400">overdue</span>}
        </div>
      )}
                  ...
                  .map(cs => {
                    ...
                    return (
                      <div key={cs.cluster}>
                        ...
                        {cs.logs!.map((line, i) => (
                          <div key={i} className="text-2xs font-mono text-muted-foreground leading-relaxed truncate flex items-start gap-1.5">
...
export function ClusterStatusRow({ status, clusterStatusConfig }: ClusterStatusRowProps) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className={cn('text-2xs shrink-0', config.color)}>{config.label}</span>
    </div>
  )
}
...
export function DependencySummary({ dependencies, depActionStyles }: { dependencies: DeployedDep[]; depActionStyles: Record<string, { color: string; label: string }> }) {
