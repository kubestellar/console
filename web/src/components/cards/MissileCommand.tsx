...
export function MissileCommand(_props: CardComponentProps) {
  ...
  const initLevel = useCallback((waveNum: number) => {
    // Place cities evenly, avoiding battery positions
    const newCities: City[] = []
    const cityPositions = [40, 80, 120, 200, 240, 280]
    for (let i = 0; i < CITY_COUNT; i++) {
      newCities.push({ x: cityPositions[i], alive: true })
    }
    setCities(newCities)

    // Place 3 batteries symmetrically
    const newBatteries: MissileBattery[] = [
      { x: 10, ammo: 10 },
      { x: CANVAS_WIDTH / 2 - MISSILE_BATTERY_WIDTH / 2, ammo: 10 },
      { x: CANVAS_WIDTH - 10 - MISSILE_BATTERY_WIDTH, ammo: 10 },
    ]
    ...
    for (let i = 0; i < count; i++) {
      ...
      newMissiles.push({
        ...
        targetX: tx,
        targetY: GROUND_Y - 4,
        speed: 0.4 + waveNum * 0.1 + Math.random() * 0.2,
        trail: [],
      })
    }
  ...
  const draw = useCallback(() => {
    ...
    for (const batt of state.batteries) {
      ...
      // Ammo indicator dots
      for (let a = 0; a < Math.min(batt.ammo, 10); a++) {
  ...
  useEffect(() => {
    ...
    gameLoopRef.current = setInterval(() => {
      ...
      setEnemyMissiles(ms =>
        ms
          .map(m => {
            ...
            if (dist < m.speed) return null // reached target
            const nx = m.x + (dx / dist) * m.speed
            const ny = m.y + (dy / dist) * m.speed
            const newTrail = [...m.trail, { x: m.x, y: m.y }].slice(-20)
        ...
        ms
          .map(m => {
            ...
            if (dist < m.speed) {
              ...
              setExplosions(exs => [
                ...exs,
                { id: nextId++, x: m.targetX, y: m.targetY, radius: 2, maxRadius: 30, growing: true },
              ])
              return null
            }
            return { ...m, x: m.x + (dx / dist) * m.speed, y: m.y + (dy / dist) * m.speed }
          })
      ...
      const curExplosions = state.explosions
      setEnemyMissiles(ms =>
        ms.filter(m => {
          for (const ex of curExplosions) {
            const dx = m.x - ex.x
            const dy = m.y - ex.y
      ...
      // Collision: enemy missiles reaching ground hit cities / batteries
      const reachedGround = state.enemyMissiles.filter(m => {
        const dy = m.targetY - m.y
        return Math.abs(dy) < m.speed + 1
      })

      if (reachedGround.length > 0) {
        for (const m of reachedGround) {
          // Create explosion on impact
          setExplosions(exs => [
            ...exs,
            { id: nextId++, x: m.targetX, y: m.targetY, radius: 2, maxRadius: 20, growing: true },
          ])
          // Destroy nearby cities
          setCities(cs =>
            cs.map(c => {
              if (!c.alive) return c
              const dx = c.x - m.targetX
              if (Math.abs(dx) < CITY_WIDTH) return { ...c, alive: false }
              return c
            })
          )
          // Drain nearby battery ammo
          setBatteries(bs =>
            bs.map(b => {
              const dx = b.x + MISSILE_BATTERY_WIDTH / 2 - m.targetX
              if (Math.abs(dx) < MISSILE_BATTERY_WIDTH + 5 && b.ammo > 0) {
                return { ...b, ammo: Math.max(0, b.ammo - 3) }
              }
              return b
            })
          )
      ...
      const remainingMissiles = state.enemyMissiles.length
      if (aliveCities === 0) {
        setGameOver(true)
        setIsPlaying(false)
        emitGameEnded('missile_command', 'loss', state.score)
        return
      }

      // Check wave complete: no enemy missiles left
      if (remainingMissiles === 0) {
        if (state.wave >= 5) {
          setWon(true)
          setGameOver(true)
          setIsPlaying(false)
          emitGameEnded('missile_command', 'win', state.score)
        } else {
          setWave(w => {
            const nextWave = w + 1
            // Bonus points for surviving cities
            setScore(s => s + aliveCities * 50)
            initLevel(nextWave)
            return nextWave
          })
        }
  ...
  }, [isPlaying, gameOver, draw, initLevel])
      ...
      <div className="flex items-center justify-between gap-2 mb-2 flex-wrap">
        ...
        <div className="flex items-center gap-3 text-xs">
          ...
          <div className="text-center">
            ...
            <div className="font-bold text-orange-400">{wave}/5</div>
          </div>
          <div className="text-center">
