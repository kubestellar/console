...
import { HIGH_USAGE_PCT, MEDIUM_USAGE_PCT, TABLE_PAGE_SIZE, type NodeTableRow, type TranslateFn } from './GPUInventoryHistory.parts'

interface GPUInventoryHistoryTableProps {
...
export function GPUInventoryHistoryTable({
  paginatedRows,
  tableRowsLength,
  tablePage,
  setTablePage,
    ...
    <div className="flex-1 overflow-auto">
      ...
      {totalTablePages > 1 && (
        <div className="flex flex-wrap items-center justify-between gap-y-2 mt-2 text-xs text-muted-foreground">
          <span>{t('cards:gpuInventoryHistory.showing', 'Showing')} {tablePage * TABLE_PAGE_SIZE + 1}-{Math.min((tablePage + 1) * TABLE_PAGE_SIZE, tableRowsLength)} {t('cards:gpuInventoryHistory.of', 'of')} {tableRowsLength}</span>
