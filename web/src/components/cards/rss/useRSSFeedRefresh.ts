...
interface UseRSSFeedRefreshParams {
  ...
  feeds: FeedConfig[]
  isDemoMode: boolean
  failedToLoadFeedText: string
  setItems: (items: FeedItem[]) => void
  setItemsSourceUrl: (url: string) => void
...
export function useRSSFeedRefresh({
  ...
  const fetchFeed = useCallback(async (isManualRefresh = false) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : failedToLoadFeedText
  ...
  }, [activeFeed?.url, activeFeed?.name, activeFeed?.isAggregate, activeFeed?.sourceUrls, isDemoMode, feeds, fetchSingleFeed, failedToLoadFeedText, setError, setFetchSuccess, setIsLoading, setIsRefreshing, setItems, setItemsSourceUrl, setLastRefresh])
