import { ChevronDown, ChevronRight, Hourglass, WifiOff } from 'lucide-react'
import { useTranslation } from 'react-i18next'
import { ClusterBadge } from '../ui/ClusterBadge'
import { NamespaceCard, NamespaceCardSkeleton } from './NamespaceCard'
import type { NamespaceDetails } from './types'
import type { ClusterNamespaceStatus } from './useNamespaceFetch'

interface NamespaceClusterGroupProps {
  clusterName: string
  namespaces: NamespaceDetails[]
  isCollapsed: boolean
  onToggleCollapse: (clusterName: string) => void
  isLoading: boolean
  clusterStatus: ClusterNamespaceStatus | undefined
  hasData: boolean
  isUnreachable: boolean
  selectedNamespace: NamespaceDetails | null
  onSelect: (namespace: NamespaceDetails) => void
  onDelete: (namespace: NamespaceDetails) => void
}
...
export function NamespaceClusterGroup({
  ...
  return (
    <div>
      ...
      <button
        onClick={() => onToggleCollapse(clusterName)}
