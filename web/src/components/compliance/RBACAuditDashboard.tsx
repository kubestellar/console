...
export const RBACAuditDashboardContent = memo(function RBACAuditDashboardContent() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center h-64">
      ...
      <span className="ml-3 text-muted-foreground">Loading RBAC audit data…</span>
    </div>
  )
      ...
      {summary && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-card/50 border border-border rounded-xl p-4">
            <div className="text-sm text-muted-foreground mb-1">Total Bindings</div>
          ...
          <div className="bg-card/50 border border-border rounded-xl p-4">
            <div className="text-sm text-muted-foreground mb-1">Over-Privileged</div>
            <div className={`text-3xl font-bold ${summary.over_privileged > 0 ? 'text-red-400' : 'text-emerald-400'}`}>
              {summary.over_privileged}
            </div>
            <div className="text-xs text-muted-foreground mt-1">bindings with excess permissions</div>
          </div>
          <div className="bg-card/50 border border-border rounded-xl p-4">
            <div className="text-sm text-muted-foreground mb-1">Unused (30d)</div>
          ...
          <div className="bg-card/50 border border-border rounded-xl p-4">
            ...
            <div className="text-xs text-muted-foreground mt-1">least-privilege adherence</div>
          </div>
        </div>
      )}
        ...
        <div className="space-y-3">
          {filteredFindings.length === 0 ? (
            ...
            filteredFindings.map(f => (
              <div key={f.id} className="bg-card/50 border border-border rounded-xl p-4">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3">
                    <ShieldAlert className="w-5 h-5 text-amber-400" />
                    <div>
                      <div className="text-foreground font-medium">{(f.finding_type ?? '').replace(/_/g, ' ')}</div>
                      <div className="text-sm text-muted-foreground">{f.subject} — {f.cluster}/{f.namespace}</div>
                    </div>
                  </div>
                ...
                <p className="text-sm text-foreground mb-2">{f.description}</p>
            ...
            <tbody>
              {bindings.map(b => (
                <tr key={b.id} className="border-b border-border/50 hover:bg-secondary/50">
                  ...
                  <td className="p-3">
                    <span className="px-2 py-1 bg-secondary/50 rounded text-xs text-muted-foreground">{b.kind}</span>
                  </td>
                  <td className="p-3 text-foreground">
                    ...
                    <span className="text-xs text-muted-foreground">{b.subject_kind}</span>
                  </td>
                  <td className="p-3 text-foreground font-mono text-xs">{b.role_name}</td>
                  <td className="p-3 text-foreground">{b.cluster}</td>
                  <td className="p-3 text-foreground">{b.namespace || '—'}</td>
                  <td className="p-3">
                    <span className={`font-medium text-xs ${RISK_STYLES[b.risk_level] || 'text-muted-foreground'}`}>
                      {b.risk_level === 'critical' ? <AlertTriangle className="w-3 h-3 inline mr-1" /> : null}
                      {b.risk_level}
                    </span>
                  </td>
                  <td className="p-3 text-muted-foreground text-xs">{new Date(b.last_used).toLocaleDateString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
