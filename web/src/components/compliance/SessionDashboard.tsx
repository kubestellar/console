    ...
    <div className="space-y-6 p-6">
      ...
      {summary && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-card/50 border border-border rounded-xl p-4">
            ...
            <div className="text-xs text-muted-foreground mt-1">with active sessions</div>
          </div>
          <div className="bg-card/50 border border-border rounded-xl p-4">
            ...
            <div className="text-xs text-muted-foreground mt-1">MFA: {summary.mfa_sessions_pct}% of sessions</div>
          </div>
          <div className="bg-card/50 border border-border rounded-xl p-4">
            ...
            <div className="text-xs text-muted-foreground mt-1">enforcement actions</div>
          </div>
        </div>
      )}
            ...
            <tbody>
              {sessions.map(s => (
                <tr key={s.id} className="border-b border-border hover:bg-muted/50">
                  ...
                  <td className="p-3 text-foreground">{s.provider}</td>
                  <td className="p-3 text-muted-foreground text-xs">{new Date(s.login_time).toLocaleString()}</td>
                  <td className="p-3 text-muted-foreground text-xs">{new Date(s.last_activity).toLocaleString()}</td>
                  <td className="p-3 text-foreground font-mono text-xs">{s.ip_address}</td>
                  <td className="p-3 text-muted-foreground text-xs truncate max-w-[150px]">
                  ...
                  <td className="p-3">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium border ${STATUS_STYLES[s.status] || 'bg-muted/50 text-muted-foreground border-border'}`}>
                      {s.status}
                    </span>
                  </td>
                </tr>
      ...
      {activeTab === 'policies' && (
        <div className="space-y-3">
          {policies.map(p => (
            <div key={p.id} className="bg-card/50 border border-border rounded-xl p-4">
              <div className="flex items-start justify-between mb-2">
                ...
                <span className="px-2 py-1 bg-secondary/50 rounded text-xs text-muted-foreground">{p.scope}</span>
              </div>
              <div className="grid grid-cols-4 gap-4 mt-3">
                <div>
                  <p className="text-xs text-muted-foreground">Idle Timeout</p>
                  <p className="text-sm font-medium text-foreground">{p.idle_timeout_minutes} min</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Absolute Timeout</p>
                  <p className="text-sm font-medium text-foreground">{p.absolute_timeout_hours} hrs</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Max Concurrent</p>
                  <p className="text-sm font-medium text-foreground">{p.max_concurrent}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Enforce MFA</p>
