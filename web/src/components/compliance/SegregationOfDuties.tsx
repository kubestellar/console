    ...
    <div className="space-y-6">
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <SummaryCard label="Principals" value={summary.total_principals} icon={<Users className="w-5 h-5 text-muted-foreground" />} />
          ...
          {filteredViolations.length === 0 ? <p className="text-muted-foreground text-sm text-center py-8">No violations found</p> : filteredViolations.map(v => (
            <div key={v.id} className="rounded-lg border border-border bg-card p-4">
              ...
              <div className="flex gap-2 text-xs">
                <span className="bg-red-500/10 text-red-400 px-2 py-0.5 rounded">{v.role_a}</span>
                <span className="text-muted-foreground">conflicts with</span>
                <span className="bg-red-500/10 text-red-400 px-2 py-0.5 rounded">{v.role_b}</span>
              </div>
            </div>
          ))}
        </div>
            ...
            return (
              <div key={p.name} className={`rounded-lg border p-3 ${hasViolation ? 'border-red-500/30 bg-red-500/5' : 'border-border bg-card'}`}>
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {(p.roles || []).map(r => <span key={r} className="text-xs bg-secondary px-1.5 py-0.5 rounded text-foreground">{r}</span>)}
                </div>
        ...
        <div className="space-y-2">
          {rules.map(r => (
            <div key={r.id} className="rounded-lg border border-border bg-card p-4">
              ...
              <div className="flex gap-2 text-xs">
                <span className="bg-secondary px-2 py-0.5 rounded text-foreground">{r.role_a}</span>
                <XCircle className="w-4 h-4 text-red-400" />
                <span className="bg-secondary px-2 py-0.5 rounded text-foreground">{r.role_b}</span>
              </div>
            </div>
          ))}
        </div>
