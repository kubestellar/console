...
interface UseDashboardCardActionsParams {
  ...
  recordCardAdded: (id: string, type: string, title?: string, config?: Record<string, unknown>, dashboardId?: string, dashboardName?: string) => void
  recordCardRemoved: (id: string, type: string, title?: string, config?: Record<string, unknown>, dashboardId?: string, dashboardName?: string) => void
  recordCardConfigured: (id: string, type: string, title?: string, config?: Record<string, unknown>, dashboardId?: string, dashboardName?: string) => void
  closeConfigureCard: () => void
  closeAddCardModal: () => void
  closeWidgetExport: () => void
  exportDashboard: (dashboardId: string) => Promise<DashboardLike>
}
...
export function useDashboardCardActions({
  ...
  const handleAddCards = useCallback(async (suggestions: SuggestedCard[]) => {
    ...
    snapshot(localCards)
    setLocalCards(prev => insertAtIndex !== null
      ? [...prev.slice(0, insertAtIndex), ...newCards, ...prev.slice(insertAtIndex)]
      : [...newCards, ...prev])
  ...
  }, [dashboard?.id, dashboard?.name, insertAtIndex, localCards, recordCardAdded, setLocalCards, showToast, snapshot, t])
  ...
  }, [dashboard?.id, dashboard?.name, localCards, recordCardRemoved, setLocalCards, snapshot])
  ...
  const handleWidthChange = useCallback(async (cardId: string, newWidth: number) => {
    ...
    setLocalCards(prev => prev.map(card => (
      card.id === cardId ? { ...card, position: { ...(card.position || { w: 4, h: 2 }), w: newWidth } } : card
    )))
    ...
    if (dashboard?.id && !isLocalOnlyCard(cardId)) {
      try {
        ...
        if (card) {
          await api.put(`/api/cards/${cardId}`, { position: { ...(card.position || { w: 4, h: 2 }), w: newWidth } })
        }
      } catch (error) {
  ...
  }, [dashboard?.id, localCards, setLocalCards, showToast, snapshot, t])

  const handleHeightChange = useCallback(async (cardId: string, newHeight: number) => {
    snapshot(localCards)
    setLocalCards(prev => prev.map(card => (
      card.id === cardId ? { ...card, position: { ...(card.position || { x: 0, y: 0, w: 4, h: 2 }), h: newHeight } } : card
    )))
    ...
    if (dashboard?.id && !isLocalOnlyCard(cardId)) {
      try {
        const card = localCards.find(item => item.id === cardId)
        if (card) {
          await api.put(`/api/cards/${cardId}`, { position: { ...(card.position || { x: 0, y: 0, w: 4, h: 2 }), h: newHeight } })
        }
      } catch (error) {
  ...
  const handleCardConfigured = useCallback(async (cardId: string, newConfig: Record<string, unknown>, newTitle?: string) => {
    ...
    snapshot(localCards)
    setLocalCards(prev => prev.map(item => (
      item.id === cardId ? { ...item, config: newConfig, title: newTitle || item.title } : item
    )))
    closeConfigureCard()
  ...
  }, [closeConfigureCard, dashboard?.id, dashboard?.name, localCards, recordCardConfigured, setLocalCards, showToast, snapshot, t])
  ...
  }, [dashboard?.id, dashboard?.name, localCards, recordCardAdded, setLocalCards, snapshot])
  ...
  const handleCreateCardFromAI = useCallback((cardType: string, config: Record<string, unknown>, title?: string) => {
    ...
    closeConfigureCard()
    setSelectedCard(null)
  }, [closeConfigureCard, dashboard?.id, dashboard?.name, localCards, recordCardAdded, setLocalCards, snapshot])
  ...
  }, [dashboard?.id, dashboard?.name, localCards, recordCardAdded, setLocalCards, showToast, snapshot, t])
  ...
  const handleInsertBefore = useCallback((index: number) => setInsertAtIndex(index), [])
  const handleInsertAfter = useCallback((index: number) => setInsertAtIndex(index + 1), [])
