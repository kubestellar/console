...
import {
  ...
  getInitialDashboardCards,
  setDashboardCache,
} from './dashboardCardState'
import { useDashboardLayoutState } from './dashboardLayoutState'
import { useDashboardFilterState } from './dashboardFilterState'

const DASHBOARD_STORAGE_KEY = getDashboardStorageKey()
const DEFAULT_DASHBOARD_CARDS = getDefaultDashboardCards()
...
export function useDashboardState() {
  ...
  const [localCards, setLocalCards] = useState<Card[]>(() => getInitialDashboardCards())
  ...
  useEffect(() => {
    if (localCards.length > 0) {
      ...
      if (currentCache) {
        setDashboardCache({ ...currentCache, cards: localCards, timestamp: Date.now() })
      }
