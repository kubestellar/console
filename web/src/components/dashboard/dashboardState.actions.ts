 ...
 * to the API.  Returns the new Card array so the caller can update state.
 */
...
export function handleDragReorder(
  ...
  const { arrayMove } = require('@dnd-kit/sortable') as typeof import('@dnd-kit/sortable')
  const draggedCard = localCards.find(card => card.id === activeId)
