...
import { Plus, X, Save, Trash2, Activity, Sparkles, CheckCircle, GripVertical, Eye, EyeOff, Maximize2, Minimize2 } from 'lucide-react'
...
export function StatBlockFactoryModal({ isOpen, onClose, onStatsCreated, embedded = false }: StatBlockFactoryModalProps) {
    ...
    startTransition(() => {
      ...
      if (result.blocks && result.blocks.length > 0) {
        setBlocks(result.blocks.map(b => ({
          ...
          color: (AVAILABLE_COLORS.includes(b.color as any) ? b.color : 'purple') as any,
          field: b.field || '',
          format: b.format || '',
          tooltip: b.tooltip || '',
        })))
  ...
  const applySmartDefault = (idx: number, defaults: any) => {
