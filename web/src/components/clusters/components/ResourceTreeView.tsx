import { ChevronRight, ChevronDown, Box, Layers, Network, Briefcase, Activity, User, HardDrive, Settings, Lock } from 'lucide-react'
import type { TFunction } from 'i18next'
import { StatusBadge } from '../../ui/StatusBadge'
import { TechnicalAcronym } from '../../shared/TechnicalAcronym'
import type { PodInfo, Deployment, Job, HPA } from '../../../hooks/mcp/types.workloads'
import type { ConfigMap, Secret, ServiceAccount, PVC } from '../../../hooks/mcp/types.resources'
import type { Service } from '../../../hooks/mcp/types'
...
const MAX_STANDALONE_PODS = 20
const MAX_DEPLOYMENT_PODS = 10
const MAX_LIST_ITEMS = 20

/**
 * Hierarchical "tree view" of namespace resources grouped by type, with
 * deployments expandable to show their owned pods. Extracted from
 * NamespaceResources.tsx to keep that file under the line/hook budget (#21617).
 */
                  ...
                  return (
                    <div key={dep.name} className="mb-0.5">
                      ...
                      {isExpanded && depPods.length > 0 && (
                        <div className="ml-4 border-l border-border/30 pl-2">
                          {depPods.slice(0, MAX_DEPLOYMENT_PODS).map(pod => (
                            <div
                              key={pod.name}
                          ...
                          {depPods.length > MAX_DEPLOYMENT_PODS && <div className="text-xs text-muted-foreground pl-5">+{depPods.length - MAX_DEPLOYMENT_PODS} more</div>}
                        </div>
                      )}
                    </div>
                  )
        ...
        {podsByDeployment.standalone.length > 0 && (
          <div className="mb-1">
            ...
            {expandedTypes.has('pods') && (
              <div className="ml-4 border-l border-border/30 pl-2">
                {podsByDeployment.standalone.slice(0, MAX_STANDALONE_PODS).map(pod => (
                  <div
                    key={pod.name}
                    className="flex items-center gap-2 min-h-11 px-1 text-xs cursor-pointer hover:bg-card/30 rounded"
                    onClick={() => onResourceClick('Pod', pod.name, pod.namespace, { status: pod.status, restarts: pod.restarts })}
                ...
                {podsByDeployment.standalone.length > MAX_STANDALONE_PODS && <div className="text-xs text-muted-foreground pl-5">+{podsByDeployment.standalone.length - MAX_STANDALONE_PODS} more</div>}
              </div>
            )}
          </div>
        )}
        ...
        {serviceAccounts.length > 0 && (
          <div className="mb-1">
            ...
            {expandedTypes.has('serviceaccounts') && (
              <div className="ml-4 border-l border-border/30 pl-2">
                {serviceAccounts.slice(0, MAX_LIST_ITEMS).map(sa => (
                  <div
                    key={sa.name}
                    className="flex items-center gap-2 min-h-11 px-1 text-xs cursor-pointer hover:bg-card/30 rounded"
                ...
                {serviceAccounts.length > MAX_LIST_ITEMS && <div className="text-xs text-muted-foreground pl-5">+{serviceAccounts.length - MAX_LIST_ITEMS} more</div>}
              </div>
            )}
          </div>
        )}
        ...
        {pvcs.length > 0 && (
          <div className="mb-1">
            ...
            {expandedTypes.has('pvcs') && (
              <div className="ml-4 border-l border-border/30 pl-2">
                {pvcs.slice(0, MAX_LIST_ITEMS).map(pvc => (
                  <div
                    key={pvc.name}
                    className="flex items-center gap-2 min-h-11 px-1 text-xs cursor-pointer hover:bg-card/30 rounded"
                ...
                {pvcs.length > MAX_LIST_ITEMS && <div className="text-xs text-muted-foreground pl-5">+{pvcs.length - MAX_LIST_ITEMS} more</div>}
              </div>
            )}
          </div>
        )}
        ...
        {configmaps.length > 0 && (
          <div className="mb-1">
            ...
            {expandedTypes.has('configmaps') && (
              <div className="ml-4 border-l border-border/30 pl-2">
                {configmaps.slice(0, MAX_LIST_ITEMS).map(cm => (
                  <div
                    key={cm.name}
                    className="flex items-center gap-2 min-h-11 px-1 text-xs cursor-pointer hover:bg-card/30 rounded"
                ...
                {configmaps.length > MAX_LIST_ITEMS && <div className="text-xs text-muted-foreground pl-5">+{configmaps.length - MAX_LIST_ITEMS} more</div>}
              </div>
            )}
          </div>
        )}
        ...
        {secrets.length > 0 && (
          <div className="mb-1">
            ...
            {expandedTypes.has('secrets') && (
              <div className="ml-4 border-l border-border/30 pl-2">
                {secrets.slice(0, MAX_LIST_ITEMS).map(secret => (
                  <div
                    key={secret.name}
                    className="flex items-center gap-2 min-h-11 px-1 text-xs cursor-pointer hover:bg-card/30 rounded"
                ...
                {secrets.length > MAX_LIST_ITEMS && <div className="text-xs text-muted-foreground pl-5">+{secrets.length - MAX_LIST_ITEMS} more</div>}
              </div>
            )}
          </div>
        )}
