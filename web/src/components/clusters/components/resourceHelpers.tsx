...
import { Box, Layers, Network, Briefcase, Activity, Settings, Lock, User, HardDrive } from 'lucide-react'

/** All Kubernetes resource kinds rendered in the namespace resource explorer. */
export type ResourceKind = 'Pod' | 'Deployment' | 'Service' | 'Job' | 'HPA' | 'ConfigMap' | 'Secret' | 'ServiceAccount' | 'PVC'

/** A single row in the flattened "list view" of namespace resources. */
export interface NamespaceResourceItem {
  kind: ResourceKind
  name: string
  namespace?: string
  status?: string
...
/**
 * Resource kind icon mapping, shared by the list and tree views of
 * NamespaceResources. Extracted from NamespaceResources.tsx (#21617).
 */
export function getKindIcon(kind: ResourceKind): ReactNode {
  switch (kind) {
...
/** Maps a resource's semantic status color to its badge background/text classes. */
