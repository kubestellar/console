...
if (isBrowserEnvironment) {
  // Mock react-i18next globally with synchronous mock to ensure consistent
  // behavior across all test contexts. Async importOriginal pattern can cause
  // timing issues where some tests don't get the mock properly initialized.
  vi.mock('react-i18next', () => ({
    ...
    useTranslation: () => ({
      t: (key: string, options?: Record<string, unknown>) => {
        ...
        // Default: return the key as a fallback (allows tests to match on translation keys)
        return key
      },
