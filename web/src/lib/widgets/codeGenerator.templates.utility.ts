...
import { generateReactCardRenderCase } from './codeGenerator.templates.react'
import { generateVueCardRenderCase } from './codeGenerator.templates.vue'
import { generateAngularCardRenderCase } from './codeGenerator.templates.angular'

export interface CardRenderShared {
  parseBlock: string
  wrapOpen: string
  wrapClose: string
  issueButton: string
...
export function generateCardRenderFunction(cardType: string, displayName?: string): string {
  ...
  const shared: CardRenderShared = { parseBlock, wrapOpen, wrapClose, issueButton }

  return generateReactCardRenderCase(cardType, card, shared)
    ?? generateVueCardRenderCase(cardType, card, shared)
    ?? generateAngularCardRenderCase(cardType, card, shared)
    ?? (() => {
