...
// Replaces each ' with the sequence '\'' (end quote, escaped quote, reopen quote).
...
export function generateWidgetCommand(baseUrl: string, curlUrl: string): string {
  ...
  const safeTokenUrl = shellEscapeSingleQuote(`${baseUrl}/api/agent/token?source=ubersicht-widget`)
