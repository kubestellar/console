...
// shellEscapeSingleQuote escapes single quotes for safe interpolation inside
// single-quoted shell strings. Each ' becomes '\'' (end quote, escaped quote, resume quote).
...
export function generateWidgetCommand(baseUrl: string, curlUrl: string): string {
  ...
  const tokenUrl = shellEscapeSingleQuote(`${baseUrl}/api/agent/token?source=ubersicht-widget`)
