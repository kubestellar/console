...
describe('cache module', () => {
  ...
  describe('clearAllInMemoryCaches', () => {
    ...
    it('calling the registered reset function does not throw', async () => {
      ...
      // Get the registered callback from registerCacheReset
      const registeredCb = (registerCacheReset as ReturnType<typeof vi.fn>).mock.calls
        .find(([name]: [string]) => name === 'unified-cache')

      if (registeredCb) {
        const resetFn = registeredCb[1] as () => void
