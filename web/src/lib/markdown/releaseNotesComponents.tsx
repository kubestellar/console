...
export function buildReleaseNotesComponents(fontSize: 'sm' | 'base' | 'lg' = 'sm') {
  return {
    ...
    h1: ({ children }: ChildrenProp) => (
      <div role="heading" aria-level={1} className="mt-6 mb-3 pt-3 pl-3 border-l-3 border-purple-500/50 border-t border-border/30 first:border-t-0 first:pt-0 first:mt-0 text-xl font-bold">{children}</div>
    ),
    h2: ({ children }: ChildrenProp) => (
      <div role="heading" aria-level={2} className="mt-6 mb-3 pt-3 pl-3 border-l-2 border-blue-500/40 border-t border-border/30 first:border-t-0 first:pt-0 first:mt-0 text-lg font-bold">{children}</div>
    ),
    h3: ({ children }: ChildrenProp) => (
      <div role="heading" aria-level={3} className="mt-5 mb-2 pt-2 border-t border-border/20 first:border-t-0 first:pt-0 first:mt-0 text-base font-semibold text-foreground">{children}</div>
    ),
    h4: ({ children }: ChildrenProp) => <div role="heading" aria-level={4} className="mt-4 mb-2 font-semibold">{children}</div>,
    h5: ({ children }: ChildrenProp) => <div role="heading" aria-level={5} className="mt-4 mb-2 font-medium">{children}</div>,
    h6: ({ children }: ChildrenProp) => <div role="heading" aria-level={6} className="mt-3 mb-2 font-medium">{children}</div>,
