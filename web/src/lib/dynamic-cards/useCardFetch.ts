...
/** Maximum concurrent useCardFetch hooks per card */
const MAX_CONCURRENT_FETCHES = 5

/** Tracks how many fetch hooks are active in the current card scope */
 ...
 * ```
 */
export function useCardFetch<T = unknown>(
...
/**
 * Reset the active fetch counter — called when a card unmounts.
 * Exported so scope.ts can wire it into the cleanup chain.
 */
export function resetCardFetchCount(): void {
  activeFetchCount = 0
}
