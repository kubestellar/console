...
import { shouldMarkBackendUnavailable, reportBackendAvailable, reportBackendUnavailable } from './backendHealthEvents'
import { reportAppError } from './errors/handleError'
import { API_BASE, BACKEND_OUTAGE_EXEMPT_PREFIXES, extractRequestPath, safeParseJsonOrNull } from './apiHelpers'
...
export async function checkBackendAvailability(forceCheck = false): Promise<boolean> {
  ...
  backendCheckPromise = (async () => {
    try {
      ...
      try {
        localStorage.setItem(BACKEND_STATUS_KEY, JSON.stringify({ available: backendAvailable, timestamp: backendLastCheckTime }))
      } catch (error: unknown) {
        reportAppError(error, {
...
export async function checkOAuthConfigured(): Promise<{ backendUp: boolean; oauthConfigured: boolean }> {
  try {
    ...
    const data = await safeParseJsonOrNull<{ oauth_configured?: boolean }>(response, '[api] /health OAuth config parse failed')
...
export function markBackendSuccess(status?: number): void {
  ...
  try {
    localStorage.setItem(BACKEND_STATUS_KEY, JSON.stringify({ available: true, timestamp: backendLastCheckTime }))
  } catch (error: unknown) {
    reportAppError(error, {
