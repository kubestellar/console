...
Stellar is the console's long-lived AI operations subsystem for **self-hosted** deployments. It extends the console from one-shot AI assistance into a **persistent operational runtime** with a dedicated `/stellar` workspace, durable notifications/tasks/watches/actions, provider routing, audit history, daily digests, and autonomous solve loops.
